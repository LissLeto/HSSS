function [labels,psi,seeds,Gar]=fundationSP( ...
...
for k=1:M*N
    [i, j]=ind2sub([M N],k);
    if seeds(i,j)==false
        candiSeeds=abs(Seed(:,1)-i/M)<search_R&abs(Seed(:,2)-j/N)<search_R;
        candidate=Seed(candiSeeds,:);
        if isempty(candidate)
            continue
        end
        self=[i/M,j/N,L(i,j),A(i,j),B(i,j)];
        D = (candidate-self).^2 * weight;
        [~, father] = min(D);
%         father=father(1);
        father_i=round(candidate(father,1)*M);father_j=round(candidate(father,2)*N);
        labels(i,j)=labels(father_i,father_j);
    end
end
end
%%%%%%%%%%
